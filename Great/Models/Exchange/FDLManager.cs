    ...
    public class FDLManager
    {
        ...
        public ExpenseAccountEVM ImportEAFromFile(string filePath, bool NotifyAsNew = true, bool ExcludeExpense = false, bool OverrideIfExist = false)
        {
            ...
            try
            {
                ...
                if (int.TryParse(fields[ApplicationSettings.ExpenseAccount.FieldNames.CdC].GetValueAsString(), out int cdc)) ea.CdC = cdc;

                string currency = fields[ApplicationSettings.ExpenseAccount.FieldNames.Currency].GetValueAsString();
                if (currency.Length > 4) ea.Currency = currency.Substring(0, 4).Trim();
                            ...
                            if (tmpEA == null)
                            {
                                ...
                                if (!ExcludeExpense)
                                {
                                    foreach (var entry in ApplicationSettings.ExpenseAccount.FieldNames.ExpenseMatrix)
                                    {
                                        ...
                                        if (double.TryParse(ConvertToInvariantDoubleString(fields[entry["Tue_Amount"]].GetValueAsString()), NumberStyles.Any, CultureInfo.InvariantCulture, out amount)) expense.TuesdayAmount = amount;

                                        if (double.TryParse(ConvertToInvariantDoubleString(fields[entry["Wed_Amount"]].GetValueAsString()), NumberStyles.Any, CultureInfo.InvariantCulture, out amount)) expense.WednesdayAmount = amount;

                                        if (double.TryParse(ConvertToInvariantDoubleString(fields[entry["Thu_Amount"]].GetValueAsString()), NumberStyles.Any, CultureInfo.InvariantCulture, out amount)) expense.ThursdayAmount = amount;

                                        if (double.TryParse(ConvertToInvariantDoubleString(fields[entry["Fri_Amount"]].GetValueAsString()), NumberStyles.Any, CultureInfo.InvariantCulture, out amount)) expense.FridayAmount = amount;

                                        if (double.TryParse(ConvertToInvariantDoubleString(fields[entry["Sat_Amount"]].GetValueAsString()), NumberStyles.Any, CultureInfo.InvariantCulture, out amount)) expense.SaturdayAmount = amount;

                                        if (double.TryParse(ConvertToInvariantDoubleString(fields[entry["Sun_Amount"]].GetValueAsString()), NumberStyles.Any, CultureInfo.InvariantCulture, out amount)) expense.SundayAmount = amount;
        ...
        public FDLEVM ImportFDL_XFAForm(string filePath, bool NotifyAsNew = true, bool ExcludeTimesheets = false, bool ExcludeFactories = false, bool OverrideIfExist = false)
        {
            ...
            try
            {
                ...
                if (long.TryParse(GetFieldValue(ApplicationSettings.FDL.XFAFieldNames.Order), out longResult)) fdl.Order = longResult;
                ...
                if (fdl.WeekNr == 0) throw new InvalidOperationException("Impossible to retrieve the week number.");
                                ...
                                if (!ExcludeTimesheets)
                                {
                                    foreach (var entry in ApplicationSettings.FDL.XFAFieldNames.TimesMatrix)
                                    {
                                        ...
                                        if (strDate != string.Empty)
                                        {
                                            ...
                                            if (TimeSpan.TryParse(GetFieldValue(entry.Value["TravelStartTimeAM"]).Replace("24", "00"), out time)) timesheet.TravelStartTimeAM_t = time;

                                            if (TimeSpan.TryParse(GetFieldValue(entry.Value["WorkStartTimeAM"]).Replace("24", "00"), out time)) timesheet.WorkStartTimeAM_t = time;

                                            if (TimeSpan.TryParse(GetFieldValue(entry.Value["WorkEndTimeAM"]).Replace("24", "00"), out time)) timesheet.WorkEndTimeAM_t = time;

                                            if (TimeSpan.TryParse(GetFieldValue(entry.Value["TravelEndTimeAM"]).Replace("24", "00"), out time)) timesheet.TravelEndTimeAM_t = time;

                                            if (TimeSpan.TryParse(GetFieldValue(entry.Value["TravelStartTimePM"]).Replace("24", "00"), out time)) timesheet.TravelStartTimePM_t = time;

                                            if (TimeSpan.TryParse(GetFieldValue(entry.Value["WorkStartTimePM"]).Replace("24", "00"), out time)) timesheet.WorkStartTimePM_t = time;

                                            if (TimeSpan.TryParse(GetFieldValue(entry.Value["WorkEndTimePM"]).Replace("24", "00"), out time)) timesheet.WorkEndTimePM_t = time;

                                            if (TimeSpan.TryParse(GetFieldValue(entry.Value["TravelEndTimePM"]).Replace("24", "00"), out time)) timesheet.TravelEndTimePM_t = time;
        ...
        public FDLEVM ImportFDL_AcroForm(string filePath, bool NotifyAsNew = true, bool ExcludeTimesheets = false, bool ExcludeFactories = false, bool OverrideIfExist = false)
        {
            ...
            try
            {
                ...
                if (long.TryParse(fields[ApplicationSettings.FDL.FieldNames.Order].GetValueAsString(), out long longResult)) fdl.Order = longResult;
                                ...
                                if (!ExcludeTimesheets)
                                {
                                    foreach (KeyValuePair<DayOfWeek, Dictionary<string, string>> entry in ApplicationSettings.FDL.FieldNames.TimesMatrix)
                                    {
                                        ...
                                        if (strDate != string.Empty)
                                        {
                                            ...
                                            if (TimeSpan.TryParse(fields[entry.Value["TravelStartTimeAM"]].GetValueAsString().Replace("24", "00"), out time)) timesheet.TravelStartTimeAM_t = time;

                                            if (TimeSpan.TryParse(fields[entry.Value["WorkStartTimeAM"]].GetValueAsString().Replace("24", "00"), out time)) timesheet.WorkStartTimeAM_t = time;

                                            if (TimeSpan.TryParse(fields[entry.Value["WorkEndTimeAM"]].GetValueAsString().Replace("24", "00"), out time)) timesheet.WorkEndTimeAM_t = time;

                                            if (TimeSpan.TryParse(fields[entry.Value["TravelEndTimeAM"]].GetValueAsString().Replace("24", "00"), out time)) timesheet.TravelEndTimeAM_t = time;

                                            if (TimeSpan.TryParse(fields[entry.Value["TravelStartTimePM"]].GetValueAsString().Replace("24", "00"), out time)) timesheet.TravelStartTimePM_t = time;

                                            if (TimeSpan.TryParse(fields[entry.Value["WorkStartTimePM"]].GetValueAsString().Replace("24", "00"), out time)) timesheet.WorkStartTimePM_t = time;

                                            if (TimeSpan.TryParse(fields[entry.Value["WorkEndTimePM"]].GetValueAsString().Replace("24", "00"), out time)) timesheet.WorkEndTimePM_t = time;

                                            if (TimeSpan.TryParse(fields[entry.Value["TravelEndTimePM"]].GetValueAsString().Replace("24", "00"), out time)) timesheet.TravelEndTimePM_t = time;
        ...
        private Dictionary<string, string> GetAcroFormFields(ExpenseAccountEVM ea, bool IsReadonly = false)
        {
            ...
            for (int i = 0; i < ApplicationSettings.ExpenseAccount.FieldNames.ExpenseMatrix.Count(); i++)
            {
                ...
                if (i < expenses.Count()) expense = expenses[i];
                ...
                if (IsReadonly) fields.Add(entry["Total"], expense.TotalAmount > 0 ? expenses[i].TotalAmount.ToString() : string.Empty);
            }
            ...
            if (ea.FDL1.Factory1?.OverrideAddressOnFDL == true) fields.Add(ApplicationSettings.ExpenseAccount.FieldNames.Address, ea.FDL1.Factory1.Address);
        ...
        public bool SendCancellationRequest(FDLEVM fdl)
        {
            ...
            foreach (string address in UserSettings.Email.Recipients.FDLCancelRequest) message.ToRecipients.Add(address);
        ...
        {
            if (file == null || filePath == string.Empty) return false;
            ...
            switch (type)
            {
                ...
                case EMessageType.EA_Accepted:
                    using (DBArchive db = new DBArchive())
                    {
                        ...
                        {
                            if (!(attachment is FileAttachment) || attachment.ContentType != ApplicationSettings.FDL.MIMEType) continue;
                ...
                if (FDL == string.Empty)
                {
                    ...
                    switch (type)
                    {
                        ...
                        case EMessageType.FDL_Rejected:
                            ...
                            if (match.Success || match.Groups.Count > 0) FDL = $"{message.DateTimeSent.Year}/{match.Groups[1].Value}";
                        ...
                        case EMessageType.EA_Rejected:
                            ...
                            if (words.Length > 1) FDL = $"{message.DateTimeSent.Year}/{words[1]}";
